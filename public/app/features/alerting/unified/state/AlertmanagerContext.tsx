...
const AlertmanagerProvider = ({ children, accessType, alertmanagerSourceName }: Props) => {
  ...
  // Fetch external configs to include in available alertmanagers
  const { data: externalConfigs } = alertmanagerApi.endpoints.listExternalAlertmanagerConfigs.useQuery();
  ...
  const availableAlertManagers = React.useMemo(() => {
    ...
    const externalConfigDataSources: AlertManagerDataSource[] = (externalConfigs || []).map((config) => ({
      name: `__grafana-converted-external-config-${config.identifier}`,
      displayName: `${config.identifier} (External Config)`,
      imgUrl: grafanaIconSvg,
      hasConfigurationAPI: false, // External configs don't support full alertmanager API
      handleGrafanaManagedAlerts: true,
    }));

    // List in order: Grafana -> External Configs -> Alertmanager Datasources
    ...
    orderedAlertManagers.push(...externalConfigDataSources);
    orderedAlertManagers.push(...datasourceAlertmanagers);

    return orderedAlertManagers;
  }, [allAvailableAlertManagers, externalConfigs]);
  ...
  // For external configs, we create a mock jsonData since they don't exist in regular datasources
  const selectedAlertmanagerConfig = React.useMemo(() => {
    if (selectedAlertmanager?.startsWith('__grafana-converted-external-config-')) {
      // External configs are view-only
