...
export function sceneVariablesSetToVariables(set: SceneVariables, keepQueryOptions?: boolean) {
  ...
  for (const variable of variables) {
    ...
    // For constant variables, ensure hide is preserved if it's hideVariable (2)
    // Constant variables should always have hide: 2 (hideVariable)
    if (variable.type === 'constant') {
      // hide: 2 (hideVariable) should be preserved for constant variables
      // Don't delete it even if it's the default
      if (!variable.hide || variable.hide !== OldVariableHide.hideVariable) {
        variable.hide = OldVariableHide.hideVariable;
      }
      // Constant variables should not have an options array
      // Remove it if it was added (e.g., from loading v1beta1 that had options)
      if (Object.prototype.hasOwnProperty.call(variable, 'options')) {
        const varWithOptions = variable as VariableModel & { options?: unknown };
        delete varWithOptions.options;
      }
    }
  }
