...
function createSceneVariableFromVariableModel(variable: TypedVariableModelV2): SceneVariable {
  ...
  if (variable.kind === defaultCustomVariableKind().kind) {
    ...
    // Transform refresh value - if undefined or invalid, default to never (0) to match backend behavior
    const refreshValue = transformVariableRefreshToEnumV1(variable.spec.refresh);
