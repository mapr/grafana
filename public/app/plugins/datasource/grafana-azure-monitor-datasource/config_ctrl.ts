...
export class AzureMonitorConfigCtrl {
  ...
  /** @ngInject */
  constructor(private backendSrv, private $q, private templateSrv, private $timeout) {
  ...
  async getWorkspaces() {
    const sameAs = this.current.jsonData.azureLogAnalyticsSameAs && this.subscriptions;
    if (!sameAs && !this.subscriptionsForLogAnalytics) {
      return;
    }
  ...
  async getSubscriptionsForLogsAnalytics() {
    ...
    this.subscriptionsForLogAnalytics = await this.azureMonitorDatasource.getSubscriptions('workspacesloganalytics');
