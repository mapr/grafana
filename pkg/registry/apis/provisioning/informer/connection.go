...
// gRPC, for the in-process server. The returned Connections carry only
// namespace, name and resourceVersion.
func NewGRPCConnectionKeysLister(store resourcepb.ResourceStoreClient) KeysLister {
	return NewGRPCKeysLister(
		store,
		provisioningapis.ConnectionResourceInfo.GroupVersionResource(),
		func(ns, name, rv string) runtime.Object {
			return &provisioningapis.Connection{ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: name, ResourceVersion: rv}}
		},
	)
}
