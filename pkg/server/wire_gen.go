...
import (
	...
	migrator2 "github.com/grafana/grafana/pkg/services/secrets/migrator"
...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	keeperConfigReader, err := metadata.ProvideKeeperConfigReader(databaseDatabase, tracer, registerer)
	if err != nil {
		return nil, err
	}
	...
	secureValueService := service5.ProvideSecureValueService(tracer, accessClient, databaseDatabase, secureValueMetadataStorage, secureValueValidator, secureValueMutator, keeperConfigReader, ossKeeperService, registerer)
	...
	worker := garbagecollectionworker.ProvideWorker(cfg, secureValueMetadataStorage, keeperConfigReader, ossKeeperService)
	...
	decryptStorage, err := metadata.ProvideDecryptStorage(tracer, ossKeeperService, keeperConfigReader, secureValueMetadataStorage, decryptAuthorizer, registerer)
	if err != nil {
		return nil, err
	}
	...
	v6 := extras.ProvideProvisioningOSSRepositoryExtras(cfg, decryptService, factory, webhookExtraBuilder)
	repositoryFactory, err := extras.ProvideFactoryFromConfig(cfg, v6)
	if err != nil {
		return nil, err
	}
