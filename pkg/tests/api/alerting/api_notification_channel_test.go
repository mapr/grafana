...
func TestNotificationChannels(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		fmt.Println("------ @@", mockChannel.totalNotifications(), len(alertNames))
...
func (nc *mockNotificationChannel) matchesExpNotifications(exp map[string][]string) bool {
	...
	if len(nc.receivedNotifications) != len(exp) {
		fmt.Println("------- ## Place 1", len(nc.receivedNotifications), len(exp))
		return false
	}
	...
	for expKey, expVals := range exp {
		...
		if !ok || len(actVals) != len(expVals) {
			fmt.Println("------- ## Place 2", ok, len(actVals), len(expVals))
			return false
		}
		for i := range expVals {
			expVal := expVals[i]
			if expKey == "webhook_recv/webhook_test" {
				...
				fmt.Println("------- ## Place 3", parts[1])
			}

			var expJson, actJson interface{}
			fmt.Println("------- ## Place 4.1", expVal)
			fmt.Println("------- ## Place 4.2", actVals[i])
