...
func (a *api) getPermissionName(perm accesscontrol.SetResourcePermissionCommand) string {
	if perm.UserID != 0 {
		userDetails, err := a.service.userService.GetByID(context.Background(), &user.GetUserByIDQuery{ID: perm.UserID})
		if err != nil {
			return ""
		}
